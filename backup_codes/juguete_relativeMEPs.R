...
emmip(
  modelo_2,
  Error_Prev ~ Block_Factor | Predictability,
  CIs = TRUE,
  labs = list(
    y = "Log(PEM)",
    x = "Erro Anterior",
...
###############################################################
#   ANÁLISE DE MODELOS MISTOS - PEMS FDS
###############################################################
...
df_clean <- df %>%
  filter(type_block == "Pulse") %>%                       # Apenas trials com pulso TMS
  select(volunteer, trial, relMean_MEPpp_FDS, context,
         last_random_was_error) %>%                       # Seleção de colunas
  drop_na() %>%                                           # Remover missings
  mutate(
    log_PEM_FDS = log(relMean_MEPpp_FDS),                     # Transformação log
...
# Modelo com intercepto aleatório por voluntário
modelo_1 <- lmer(
  log_PEM_FDS ~ Predictability * Error_Prev + (1 | volunteer),
  data = df_clean,
  REML = TRUE
)
...
df_clean <- df %>%
  filter(type_block == "Pulse") %>%
  select(volunteer, trial, block_info, relMean_MEPpp_FDS,
         context, last_random_was_error) %>%
  drop_na() %>%
  mutate(
    log_PEM_FDS = log(relMean_MEPpp_FDS),
...
modelo_2 <- lmer(
  log_PEM_FDS ~ Predictability * Error_Prev * Block_Factor +
    (1 | volunteer),
  data = df_clean,
  REML = TRUE
)
