...
"""
# Setting flags
volunteer_number = 'V02'  # Change volunteer number for analysis
...
detection_blocks = 'auto' # Choose between (auto or manual)
...
# ------------ 29/11/2024 ------------
data_filtered = import_signal.import_brainvision_data('/home/victormoraes/Downloads/test_markers/testemarkers.vhdr')
if bool_plots:
    data_filtered.plot()
# -----------------------------------
...
====================================================
"""
# Split blocks and samples by annotations (D3) or manual markers
if detection_blocks == 'auto':
    (
...
if detection_blocks == 'manual':
    marker_times_sec = [127.333, 1145.934,     # Update values for start and end of each block
                        1328.03, 2259.138,
                        2299.055, 3248.307,
                        3382.719, 4316.765,
                        4347.994, 5307.438, 
                        5518.298, 6432.501]  # Times in seconds

    # Add new annotations directly
    data_filtered.set_annotations(data_filtered.annotations + mne.Annotations(
        onset=marker_times_sec,
        duration=[0],
        description=["block_marker"],
        orig_time=data_filtered.annotations.orig_time
    ))
    ...
    event_segments = signal_processing.split_events_into_blocks_manually(events_from_annot, max_segments=11)

    # Unpack each segment up to event_segment_10
    (event_segment_1, event_segment_2, event_segment_3, event_segment_4, 
    event_segment_5, event_segment_6, event_segment_7, event_segment_8, 
    event_segment_9, event_segment_10, event_segment_11) = event_segments

    # Get events from annotations based on manual markers - May need to be adjusted (see in jupyter variables)
    events_from_annot_block1 = event_segment_2
    events_from_annot_block2 = event_segment_4
    events_from_annot_block3 = event_segment_5 
    events_from_annot_block4 = event_segment_7
    events_from_annot_block5 = event_segment_8
    events_from_annot_block6 = event_segment_10
...
events_from_annot_D1 = utils.extract_events_from_annot(events_from_annot, 3)
...
if bool_plots:
    plot_data.plot_mep_vs_rms(rmsAmplitude_FDI_V, rmsAmplitude_FDS_V, MEPpp_FDI_V, MEPpp_FDS_V)

# Indices where MEPs were excluded
excluded_indices_FDI = np.where(~np.isnan(MEPpp_FDI_withExclusions_V))[0]
excluded_indices_FDS = np.where(~np.isnan(MEPpp_FDS_withExclusions_V))[0]

# Plot MEP vs RMS for excluded data
if bool_plots:
    plot_data.plot_excluded_mep_vs_rms(
        rmsAmplitude_FDI_V, rmsAmplitude_FDS_V, MEPpp_FDI_withExclusions_V, MEPpp_FDS_withExclusions_V, excluded_indices_FDI,
        excluded_indices_FDS)
...
block_info = np.concatenate((block_id_block2, block_id_block4, block_id_block6))
